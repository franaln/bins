...
LEFT_MARGIN=0
RIGHT_MARGIN=
TITLE_BAR_HEIGHT=26 #works best if all the windows are border less and this is set to zero

#either set these in a file or calculate them from the screen properties
MASTER_WIDTH=xres=`xdpyinfo | grep 'dimensions:' | awk '{print $2}' | cut -dx -f1`
MASTER_HEIGHT=`xdpyinfo | grep 'dimensions:' | awk '{print $2}' | cut -dx -f2`
